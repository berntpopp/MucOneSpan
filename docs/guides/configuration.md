...
| `hybrid.phase_single_event_alpha` | `0.001` | A single-event split selects each allele's reads by the event, so the event's read support is conditional on the split. The split is made only when the one-sided lower confidence bound (at this level; profile likelihood) of the stutter-deconvolved minor share reaches `het_af_min`, computed over the site-table sample of at most `phase_max_site_reads` reads (sampled with `seed`). The fixed sample keeps the bound's power independent of depth, since a systematic artefact does not shrink with depth. Otherwise the site stays `unresolved_single_site` (INCONCLUSIVE, located). Number strictly in (0,1). |
